...
resource "aws_service_discovery_service" "zammad_websocket" {
  name         = local.zammad_websocket_container_name
...
resource "aws_ecs_task_definition" "zammad_websocket" {
  family                   = local.zammad_websocket_container_name
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  execution_role_arn       = module.ecs_iam_zammad.role.execution.arn
  task_role_arn            = module.ecs_iam_zammad.role.task.arn
  container_definitions    = jsonencode([local.zammad_websocket_container])
...
resource "aws_ecs_service" "zammad_websocket" {
  name            = local.zammad_websocket_container_name
  cluster         = module.ecs_cluster.cluster.id
  task_definition = aws_ecs_task_definition.zammad_websocket.arn
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.zammad_websocket.arn
  }
